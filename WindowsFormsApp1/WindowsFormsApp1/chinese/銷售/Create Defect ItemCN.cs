...
namespace Better_Limited {
    public partial class frmCreateDefectCN : Form
    {
        public frmCreateDefectCN(string employeeID, string position)
        {
            InitializeComponent();
        ...
        private void frmCreateDefectCN_Load(object sender, EventArgs e)
        {
        ...
        private void btnBack_Click(object sender, EventArgs e)
        {
            ...
            frmCheckDefectCN checkDefect = new frmCheckDefectCN(employeeID, position);
            checkDefect.Show();
            this.Hide();
        }
        ...
        private void btnFinish_Click(object sender, EventArgs e)
        {
            if (checkOld.Checked == true)
            {
                ...
                if (!isNumber)
                {
                    MessageBox.Show("物品数量必须是整数");
                }
                else if (!isDouble)
                {
                    MessageBox.Show("物品金额必须是数字");
                }
                else
                {
                    InsertRecord();
                    InsertOld();
                    MessageBox.Show("一个新的缺陷物品创建成功");
                    string employeeID = txtEmpID.Text;
                    string position = txtPosition.Text;
                    frmCheckDefectCN checkDefect = new frmCheckDefectCN(employeeID, position);
                    checkDefect.Show();
