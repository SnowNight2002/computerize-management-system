...
{
    public partial class frmCheckDelivery : Form
    {
        public frmCheckDelivery(string employeeID, string position)
        {
            InitializeComponent();
        ...
        private void dataGridView1_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            using (var deliveryContext = new WindowsFormsApp1.better_limitedEntities())
            {
                ...
                if (e.ColumnIndex == 9)
                {
                    ...
                    frmDeliveryDetail detail = new frmDeliveryDetail(deliveryID, employeeID, position);
                    detail.Show();
                    this.Hide();
                }
                if (e.ColumnIndex == 10)
                {
                    DialogResult dialogResult = MessageBox.Show("Do you want to delete a delivery record?", "Information", MessageBoxButtons.YesNo);
                    if (dialogResult == DialogResult.Yes)
                    {
                        ...
                        frmCheckDelivery delivery = new frmCheckDelivery(employeeID,position);
                        delivery.Show();
                        this.Hide();
                    }
                    return;
